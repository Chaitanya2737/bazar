    ...
    <section className="transition-all duration-500 bg-gradient-to-br from-[#f8fafc] via-[#e2e8f0] to-[#cbd5e1] dark:from-[#0f172a] dark:via-[#1e293b] dark:to-[#334155] grid place-items-center px-4 py-10 font-sans">
      <div className="bg-white/70 dark:animate-gradient-dark dark:bg-white/10 dark:backdrop-blur-md backdrop-blur-xl rounded-2xl shadow-2xl max-w-4xl w-full grid md:grid-cols-2 gap-8 p-8 md:p-12 animate-fadeInUp">
        ...
        <div className="grid place-items-center w-full">
          {icon ? (
            <div className="relative w-64 aspect-video bg-white dark:bg-slate-800 rounded-xl shadow-lg overflow-hidden">
              <Image
                src={icon}
        ...
        <div className="space-y-5 text-gray-900 dark:text-white md:pl-10 md:ml-6 md:border-l-2 md:border-gray-200 dark:md:border-gray-700">
          ...
          {handlerName && (
            <p className="text-sm text-gray-700 dark:text-gray-300 animate-fadeIn">
              ...
              <span className="hover:underline cursor-pointer text-lg text--900 dark:text-white">
                {handlerName}
              </span>
            </p>
          )}
          ...
          {Array.isArray(mobileNumber) && mobileNumber.length > 0 && (
            <div className="flex items-center text-sm font-medium text-gray-700 dark:text-gray-300 animate-fadeIn">
              <Phone className="w-8 h-8 mr-2 text-indigo-500 dark:text-indigo-300" />
              <span className="space-x-2 flex flex-wrap gap-2">
                {mobileNumber.map((number, index) => (
                  <a
                    key={index}
                    href={`tel:${number}`}
                    className="relative inline-block px-2 py-1 transition-all duration-300 transform bg-gray-100 dark:bg-gray-800 rounded-md hover:scale-105 hover:text-indigo-600 dark:hover:text-indigo-400 hover:shadow-lg hover:bg-white dark:hover:bg-gray-700"
                  >
                    {number}
                  </a>
                ))}
